...
6. **Root Directory:** Déjalo vacío o `./` — la app está en la raíz del repo.
7. **Build Command:** `npm run build` (por defecto).
8. No hagas deploy todavía — primero configura las variables en el siguiente paso.

> **Si antes tenías 404:** Si tenías `Root Directory = 3ifit-app`, cámbialo a vacío (o `.`) porque la app ahora está en la raíz.
...
### 404 NOT_FOUND al abrir la app
- **Root Directory:** En Vercel → Settings → General → Root Directory. Debe estar **vacío** o `.` si la app está en la raíz del repo. Si tenías `3ifit-app`, cámbialo.
- **Estructura del repo:** La app debe tener `package.json`, `src/`, `vercel.json` en la raíz de lo que Vercel despliega.
