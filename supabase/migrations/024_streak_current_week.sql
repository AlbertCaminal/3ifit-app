-- Registrar la semana ACTUAL cuando se cumple el plan (no solo la anterior)
-- Así la racha cuenta desde la primera semana completada
...
BEGIN
  ...
  -- 1. Registrar semana ANTERIOR si se completó y aún no está registrada
  SELECT EXISTS (
  ...
  -- 2. Registrar semana ACTUAL si se completó (primera semana o cualquier semana en curso)
  SELECT EXISTS (
    SELECT 1 FROM weekly_plan_completions
  ...
  -- 3. Racha: contar semanas consecutivas. Empezar por la actual si está completada, si no por la anterior
...
-- Actualizar get_perfect_streak_weeks para incluir semana actual
...
DECLARE
  ...
  v_days_total INTEGER := 3;
  v_plan TEXT;
  v_current_completed BOOLEAN := FALSE;
BEGIN
  IF p_user_id IS NULL OR p_user_id != auth.uid() THEN
    RETURN 0;
  END IF;
  ...
  SELECT COALESCE(plan, 'estandar') INTO v_plan FROM profiles WHERE id = p_user_id;
  v_days_total := CASE v_plan WHEN 'basico' THEN 2 WHEN 'estandar' THEN 3 WHEN 'pro' THEN 5 ELSE 3 END;
  ...
  SELECT EXISTS (SELECT 1 FROM weekly_plan_completions WHERE user_id = p_user_id AND week_start = v_week_start)
    INTO v_current_completed;
  v_current_completed := v_current_completed OR (v_days_completed >= v_days_total);
