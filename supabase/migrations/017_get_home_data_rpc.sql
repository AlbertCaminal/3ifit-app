-- RPC consolidado para datos de Home: 1 round-trip en lugar de 6-8 queries
...
BEGIN
  ...
  v_four_weeks_ago := v_week_start - 28;
  ...
  SELECT ARRAY_AGG(week_start::TEXT ORDER BY week_start DESC) INTO v_week_starts
  FROM weekly_plan_completions
  WHERE user_id = p_user_id AND week_start >= v_four_weeks_ago;

  v_expected := v_week_start - 7;
  FOR i IN 1..4 LOOP
    IF v_week_starts IS NOT NULL AND v_expected::TEXT = ANY(v_week_starts) THEN
...
GRANT EXECUTE ON FUNCTION public.get_home_data(UUID) TO authenticated;
